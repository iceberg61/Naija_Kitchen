...
// Pull 6 dishes from menuData for the gallery strip — one from each category + extras
const galleryDishes = [
  menuData.find((d) => d.id === 5),  // Jollof Rice — feature (wide)
  menuData.find((d) => d.id === 1),  // Egusi Soup
  menuData.find((d) => d.id === 11), // Suya Skewers
  menuData.find((d) => d.id === 4),  // Pepper Soup
  menuData.find((d) => d.id === 9),  // Puff Puff
  menuData.find((d) => d.id === 2),  // Banga Soup
].filter(Boolean);
...
export default function CateringPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
              ...
              >
                ...
                <a
                  ...
                  className={`block text-center py-3 rounded-full font-semibold text-sm transition-colors ${
                    pkg.highlight ? "bg-white text-red-800 hover:bg-red-50" : "bg-red-700 text-white hover:bg-red-900"
                  }`}
                >
                  Book This Package
                </a>
      ...
      {/* ── Gallery strip — images from menuData ─────────────── */}
      <div className="py-16 px-4 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {galleryDishes.map((dish, i) => (
            <div
              key={dish.id}
              className={`overflow-hidden rounded-2xl group relative ${i === 0 ? "md:col-span-2 row-span-2" : ""}`}
            >
              <img
                src={dish.img}
                alt={dish.name}
                className={`w-full object-cover group-hover:scale-105 transition-transform duration-500 ${
                  i === 0 ? "h-56 md:h-full" : "h-40 md:h-48"
                }`}
                loading="lazy"
              />
              {/* Dish name overlay */}
              <div className="absolute inset-0 bg-linear-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                <div>
                  ...
                  <p className="text-white/70 text-xs">{dish.category}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {submitted ? (
            <div className="text-center py-16 px-6 bg-green-50 rounded-2xl border border-green-100">
              ...
              <p className="text-gray-500 text-sm max-w-sm mx-auto">
                Thanks <strong>{form.name}</strong>! We'll reach out to you at <strong>{form.email}</strong> within 24 hours.
              </p>
              <button
            ...
            <div className="bg-gray-50 rounded-2xl p-6 md:p-8 border border-gray-100 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <input name="name" value={form.name} onChange={handleChange} placeholder="Chukwuemeka Obi"
                ...
                <div>
                  ...
                  <input name="email" value={form.email} onChange={handleChange} type="email" placeholder="chukwu@email.com"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <input name="phone" value={form.phone} onChange={handleChange} type="tel" placeholder="080XXXXXXXX"
              ...
              <div>
                ...
                <input name="guests" value={form.guests} onChange={handleChange} type="number" min="1" placeholder="e.g. 120"
